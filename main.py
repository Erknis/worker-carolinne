...
def device_reply(text: str) -> str:
    ...
    if "fire" in t or "android tv" in t or "google tv" in t or "tv box" in t:
        return "Perfeito 😊\nNesse aparelho usamos o Downloader.\n\nVocê já tem ele instalado?"
    ...
    if "android" in t or "celular" in t or "telefone" in t:
        return "No telefone Android dá pra instalar direto pelo link 😊\n\nMe confirma só: você está nos EUA/Canadá ou em outro país?"
...
def rules_reply(req: EvolutionInbound) -> WorkerReply:
    ...
    if intent == "gerar_teste":
        return WorkerReply(intent=intent, human_handoff=True, handoff_reason="cliente pediu teste; link interno não deve ser exposto", reply_text="Claro 😊\nEu gero o teste por aqui pra você.\n\nMe diz só em qual aparelho vai usar: Smart TV, Fire Stick, celular ou TV Box?")
    if intent == "planos":
    ...
    if intent == "apps":
        return WorkerReply(intent=intent, reply_text=device_reply(req.text))
    if intent == "canais":
    ...
    handoff = count >= 2
    if handoff:
        return WorkerReply(intent="fallback", human_handoff=True, handoff_reason="fallback repetido", reply_text="Certo, vou direcionar seu atendimento para o setor responsável 😊")
...
- Para pedido de teste, responda curto: diga que gera por aqui e pergunte o aparelho.
...
def sanitize_reply(req: EvolutionInbound, reply: WorkerReply) -> WorkerReply:
    ...
    leaked = any(re.search(p, text, flags=re.I) for p in forbidden_patterns)
    if leaked:
        reply.reply_text = "Claro 😊\nEu gero o teste por aqui pra você.\n\nMe diz só em qual aparelho vai usar?"
        reply.intent = "gerar_teste"
        reply.human_handoff = True
        reply.handoff_reason = "link interno de teste bloqueado pelo filtro"
